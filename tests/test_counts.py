def test_counts(test_app, menu, submenu, dish, dish2):
    ...
    menu = test_app.get('/api/v1/menus')
    assert menu.json()[0]['submenus_count'] == 1
    assert menu.json()[0]['dishes_count'] == 2

    submenu = test_app.get(f'/api/v1/menus/{menu_id}/submenus')
    assert submenu.json()[0]['dishes_count'] == 2
    ...
    menu = test_app.get('/api/v1/menus')
    assert menu.json()[0]['submenus_count'] == 0
    assert menu.json()[0]['dishes_count'] == 0
